...
static bool mspProcessOutCommand(int16_t cmdMSP, sbuf_t *dst)
{
    ...
    case MSP_RX_CONFIG:
        ...
        sbufWriteU8(dst, 0); //was sbufWriteU8(dst, rxConfig()->rcSmoothingChannels);
    ...
    case MSP_PID_ADVANCED:
        ...
        sbufWriteU8(dst, currentPidProfile->ff_averaging);
...
static mspResult_e mspProcessInCommand(mspDescriptor_t srcDesc, int16_t cmdMSP, sbuf_t *src)
{
    ...
    case MSP_SET_PID_ADVANCED:
        ...
        if (sbufBytesRemaining(src) >= 5) {
            // Added in MSP API 1.44
#if defined(USE_FEEDFORWARD)
            currentPidProfile->ff_averaging = sbufReadU8(src);
    ...
    case MSP_SET_RX_CONFIG:
        ...
        if (sbufBytesRemaining(src) >= 6) {
            // Added in MSP API 1.40
            sbufReadU8(src); //was rxConfigMutable()->rcSmoothingChannels = sbufReadU8(src);
#if defined(USE_RC_SMOOTHING_FILTER)
