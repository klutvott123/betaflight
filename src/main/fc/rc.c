...
FAST_CODE_NOINLINE void rcSmoothingSetFilterCutoffs(rcSmoothingFilter_t *smoothingData)
{
    ...
    if ((smoothingData->setpointCutoffFrequency != oldCutoff) || !smoothingData->filterInitialized) {
        for (int i = 0; i < PRIMARY_CHANNEL_COUNT; i++) {
            if (i < (PRIMARY_CHANNEL_COUNT - 1)) {  // update setpoint filter
                ...
                if (!smoothingData->filterInitialized) {
                    pt3FilterInit((pt3Filter_t*) &smoothingData->filter[i], pt3FilterGain(smoothingData->throttleCutoffFrequency, dT));
                } else {
                    pt3FilterUpdateCutoff((pt3Filter_t*) &smoothingData->filter[i], pt3FilterGain(smoothingData->throttleCutoffFrequency, dT));
                }
            }
        }
    }
    ...
    if (rcSmoothingData.ffCutoffSetting == 0) {
        smoothingData->ffCutoffFrequency = calcAutoSmoothingCutoff(smoothingData->averageFrameTimeUs, smoothingData->autoSmoothnessFactorSetpoint);
    }
    if (!smoothingData->filterInitialized) {
...
static FAST_CODE uint8_t processRcSmoothingFilter(void)
{
    ...
    if (isRxDataNew) {
        ...
        if (calculateCutoffs) {
            ...
            // If the filter cutoffs in auto mosde, and we have good rx data, then determine the average rx frame rate
